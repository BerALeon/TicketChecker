...
public class TicketController : ControllerBase
{
    ...
    public IActionResult Validate([FromBody] TicketRequest request)
    {
        ...
        // Dummy validation logic: if folio starts with 'X', consider it invalid
        if (normalizedFolio.StartsWith("X") ||
            normalizedFolio.Contains("NOVALIDO") || 
            normalizedFolio.Contains("INVALIDO"))
        {
            return Ok(new TicketResponse 
            { 
                Status = "INVALID", 
